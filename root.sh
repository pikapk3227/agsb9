...
tee /etc/apt/sources.list << SOURCES
deb http://us-west1.gce.archive.ubuntu.com/ubuntu jammy main universe restricted multiverse
deb http://us-west1.gce.archive.ubuntu.com/ubuntu jammy-updates main universe restricted multiverse
deb http://us-west1.gce.archive.ubuntu.com/ubuntu jammy-backports main universe restricted multiverse
deb http://security.ubuntu.com/ubuntu jammy-security main universe restricted multiverse
SOURCES
...
# 更新系统并安装软件包
apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y --no-upgrade curl git zsh tmux expect sshpass locales jq passwd dropbear lsof autossh ca-certificates sudo

ln -s /usr/bin/python3 /usr/bin/python


echo -e "\033[1;32mrun setup-server-streamlit.sh and setup-ssh-cf-in-proot.sh \033[0m"

cd /root

export GIT_TOKEN="${GIT_TOKEN}"; bash <(curl -fsSL --retry 3 -H "Authorization: token ${GIT_TOKEN}" https://raw.githubusercontent.com/hhsw2015/idx-cloud/refs/heads/main/scripts/setup-server-streamlit.sh)

DEBIAN_FRONTEND=noninteractive apt-get install -y --no-upgrade wget vim nano htop python3 python3-pip nodejs npm net-tools zip unzip tree gnupg lsb-release iproute2 cron
...
cat $HOME/streamlit_ssh_info.txt

url=$(cat $HOME/ssh_upload_url.txt)
url="https://file.zmkk.fun${url}"
